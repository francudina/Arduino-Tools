...
void deviceTimeSync(Client& client) {
    ...
    HttpRequestHandler timeSyncClient(client, mainServer, mainServerPort);
    const DynamicJsonDocument doc = timeSyncClient.performGetRequestAndGetData(mainServerTimeSyncResource, 20);
    timeSyncClient.stop();
